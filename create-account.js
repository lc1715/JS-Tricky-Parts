...
//given a PIN number and an initial deposit amount. The return value should be 
...
function createAccount(pin, amount = 0) {
    return {
        ...
        deposit(pinNumber, moneyAmount) {
            if (pinNumber === pin) {
                amount += moneyAmount;
                return `Succesfully deposited $${moneyAmount}. Current balance: $${amount}.`;
            } else return 'Invalid PIN.';
        },
        withdraw(pinNumber, moneyAmount) {
            if (pinNumber === pin) {
                if (moneyAmount > amount) {
                    return "Withdrawal amount exceeds account balance. Transaction cancelled.";
                }
                amount -= moneyAmount;
                return `Succesfully withdrew $${moneyAmount}. Current balance: $${amount}.`;
            } else return 'Invalid PIN.';
        },
        changePin(pinNumber, newPinNumber) {
            if (pinNumber === pin) {
                pin = newPinNumber;
